...
(() => {
  ...
  function monitor(m){const d=m.details||{};if(d.kind!=='MONITOR')return '';return `<table class="reading-table" role="table"><caption>${t('monitoring')} · ${bdi(d.metric)} · ${bdi(d.unit)} · ${bdi(state.zone)}</caption><thead><tr><th scope="col">${t('slot')}</th><th scope="col">${t('reading')}</th><th scope="col">${t('source')}</th></tr></thead><tbody>${(d.slots||[]).map((slot,index)=>{const r=(d.readings||[]).findLast(r=>r.slot===index);return `<tr role="row"><th role="rowheader" scope="row">${time(slot)}</th><td role="cell"><span class="stack-label">${t('reading')}</span>${r?bdi(r.value)+' '+bdi(d.unit):badge(+new Date(slot)>Date.now()?'not_yet_due':'missing')}</td><td role="cell"><span class="stack-label">${t('source')}</span>${r?`${bdi(r.source_ref?.id)} · v${bdi(r.source_ref?.version)}<small>${t('received')}: ${time(r.received_at)}</small>`:t('missing')}</td></tr>`;}).join('')}</tbody></table>${(d.readings||[]).some(r=>r.slot===null)?`<h3>${t('extras')}</h3>${d.readings.filter(r=>r.slot===null).map(r=>`<p>${bdi(r.value)} ${bdi(d.unit)} ${time(r.observed_at)}</p>`).join('')}`:''}`;}
  function detail(record){
    ...
    const ordersHTML=orders.map(o=>`<article class="record-item"><p>${instruction(o.current_version||{})}</p><small>${t('version')} ${esc(o.current_version?.order_version)} · ${t(o.status)}</small>${provenance(o.current_version?.provenance)}${!demo?`<button data-amend="${esc(o.id)}">Amend instruction</button>`:''}<details><summary>${t('history')}</summary>${(o.history||[]).filter(h=>h.id!==o.current_version?.id).map(h=>`<p>${t('historical')} · ${instruction(h)}</p>${provenance(h.provenance)}`).join('')||empty()}</details></article>`).join('')||empty('no_plan');
  ...
  function correctionControls(record) {
    ...
    const change=(kind,value)=>{
      let html=`<p>Original retained: ${bdi(kind==='evidence'?value.extracted_values.map(v=>`${v.name||v.analyte}: ${v.value||''} ${v.unit||''}`).join('; '):value.payload.text)}</p><label>Action<select name="operation"><option value="replace">Correct a value</option><option value="detach">Detach from current record</option></select></label>`;
      if(kind==='evidence') html+=`<label>Reading<select name="row_index">${value.extracted_values.map((v,i)=>`<option value="${i}">${esc(v.name||v.analyte)}</option>`).join('')}</select></label><label>Correct value<input name="value" value="${esc(value.extracted_values[0]?.value||'')}"></label><label>Unit<input name="unit" value="${esc(value.extracted_values[0]?.unit||'')}"></label>`;
