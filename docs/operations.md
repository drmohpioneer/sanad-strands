...
| Image digest | `sha256:847aae8b6a8d020508cd223d228dc8dcc1cf8c572d4dfb668257df9e27bd38fd` (checkpoint `654cfff`, deployed 2026-09-11 16:43 with all seven smoke groups passing including the live Gemini smoke, first cold request 9.17 s; previous image `sha256:c4a7faa1…` from checkpoint `c831929`, 2026-09-08). It carries slices 13, 16a, 16b, 11g and 11h plus the doctor-language fix. **Three images were needed for that one fix**: `e955233` at `525d62f` rendered the card in English but saved the proposal's language from the raw preference; `315a454` at `66b3664` fixed five sites in the Scribe turn and added a source rail over that module; neither changed what a doctor received, because `card_intents` re-read the raw preference from `accounts.language()` and re-rendered the finished English card into Arabic. The override now lives in that accessor, and the guard is a test on the delivered message rather than on the source. |
