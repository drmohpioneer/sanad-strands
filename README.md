...
**Status: slices 00 to 03 accepted (2026-09-06): typed foundation, domain transitions, conditional store, and the Steward with crash-safe processing, urgent path, dispatch and sweep. Safety kernel in progress.** The package includes typed domain transitions, conditional memory/DynamoDB storage, fenced processing, recovery sweeps and delivery through a captured transport. The full care workflow and deployment remain planned. This is the single active project folder.
